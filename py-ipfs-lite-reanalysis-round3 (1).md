...
1. **Forges a record**: signs a malicious value with an attacker's private key, injects it into the mock DHT under the *legitimate* publisher's routing key (simulating a malicious/compromised DHT peer)
1. Injects an **expired** record
