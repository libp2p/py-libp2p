...
def get_parser() -> argparse.ArgumentParser:
    ...
    common_parser.add_argument("--port", type=int, default=cli_defaults.port, help="Port to listen on")
    common_parser.add_argument("--seed", type=str, default=cli_defaults.seed, help="Seed for deterministic peer ID")
    common_parser.add_argument("--offline", action="store_true", default=core_defaults.offline, help="Run in offline mode")
    common_parser.add_argument("--blockstore-type", type=str, default=core_defaults.blockstore_type, choices=["memory", "filesystem"])
    common_parser.add_argument("--blockstore-path", type=str, default=core_defaults.blockstore_path)
    common_parser.add_argument("--no-ipni", action="store_false", dest="use_ipni", default=core_defaults.use_ipni, help="Disable IPNI HTTP delegated routing")
    common_parser.add_argument("--ipni-endpoint", type=str, default=core_defaults.ipni_endpoint, help="IPNI endpoint URL")

    subparsers = parser.add_subparsers(dest="command", required=True)

    # Daemon command
    daemon_parser = subparsers.add_parser("daemon", help="Run the IPFS Lite daemon", parents=[common_parser])
    ...
    # Add command
    add_parser = subparsers.add_parser("add", help="Add a file", parents=[common_parser])
    ...
    # Get command
    get_parser = subparsers.add_parser("get", help="Get a file by CID", parents=[common_parser])
    ...
    # dag-export command
    dag_export_parser = subparsers.add_parser("dag-export", help="Export a DAG to a CAR file", parents=[common_parser])
    ...
    # dag-import command
    dag_import_parser = subparsers.add_parser("dag-import", help="Import a CAR file into the blockstore", parents=[common_parser])
