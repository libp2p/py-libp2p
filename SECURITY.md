...
2. **IPNI Announce is Unauthenticated**: IPNI `provide()` is a no-op because the protocol requires Reframe or signed provider records. Anyone can announce a CID, but block integrity is always verified on fetch.
3. **Transport Security is Noise-Only**: SECIO is fully removed from the upstream stack. All connections use Noise (XX pattern) for transport layer encryption and authentication.
4. **HTTP API is Unauthenticated**: The daemon's HTTP API (`--api-port 5001`) has no built-in auth. It binds to `127.0.0.1` by default and should never be exposed to the public internet without a reverse proxy.
