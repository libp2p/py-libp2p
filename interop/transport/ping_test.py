...
class PingTest:
    ...
    def _build_quic_addr(self, ip_value: str, port: int) -> multiaddr.Multiaddr:
        ...
        base = multiaddr.Multiaddr(
            f"/ip6/{ip_value}/udp/{port}" if is_ipv6 else f"/ip4/{ip_value}/udp/{port}"
        )
    ...
    def create_listen_addresses(self, port: int = 0) -> list:
        ...
        if self.transport == "quic-v1":
            ...
            return (
                quic_addrs if quic_addrs else [self._build_quic_addr("0.0.0.0", port)]
            )
            ...
            return (
                ws_addrs
                if ws_addrs
                else [multiaddr.Multiaddr(f"/ip4/0.0.0.0/tcp/{port}/ws")]
            )
    ...
    def log_protocols(self) -> None:
        ...
        try:
            ...
            print(
                f"Registered protocols: {[str(p) for p in protocols if p is not None]}",
                file=sys.stderr,
            )
        except Exception as e:
    ...
    async def run_listener(self) -> None:
        ...
        # Use get_available_interfaces() for proper address handling
    ...
    def _debug_connection_state(self, network, peer_id) -> None:
        ...
        try:
            ...
            for i, conn in enumerate(connections):
                ...
                if muxed:
                    try:
                        muxer_name = type(conn.get_muxer()).__name__
                        print(
                            f"[DEBUG] Connection {i} muxer: {muxer_name}",
                            file=sys.stderr,
                        )
                    except Exception as e:
                        print(
    ...
    async def run_dialer(self) -> None:
        ...
        try:
            ...
            # WS dialer workaround: need listen addresses to register
