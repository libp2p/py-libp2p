...
| Hello runtime / wire behavior              | `node/hello/hello.go:43`                     | `src/libp2p/hello/behaviour.rs:18`                        | N/A                                                | missing     | Protocol ID is shipped, but no hello request/response runtime exists. |
...
| Chain exchange request/response behavior   | `chain/exchange/server.go:32`                | `src/libp2p/chain_exchange/behaviour.rs:18`               | N/A                                                | missing     | No chain exchange codec/service/request path in py-libp2p yet.        |
| Gossipsub protocol negotiation set         | `node/modules/lp2p/pubsub.go:24`             | `src/libp2p/behaviour.rs:70`                              | `libp2p/filecoin/pubsub.py:27`                     | implemented | Preset exposes `/meshsub/2.0.0`, `1.2.0`, `1.1.0`, `1.0.0`.           |
| Blocks topic format                        | `build/params_shared_funcs.go:16`            | `src/libp2p/service.rs:75`                                | `libp2p/filecoin/constants.py:26`                  | implemented | `/fil/blocks/<network>`.                                              |
| Messages topic format                      | `build/params_shared_funcs.go:17`            | `src/libp2p/service.rs:77`                                | `libp2p/filecoin/constants.py:30`                  | implemented | `/fil/msgs/<network>`.                                                |
| DHT protocol name format                   | `build/params_shared_funcs.go:18`            | N/A (not contradicted in provided snapshot)               | `libp2p/filecoin/constants.py:34`                  | implemented | Default follows pinned Lotus helper form.                             |
...
| Calibnet alias/genesis name                | `build/buildconstants/params_calibnet.go:27` | `src/networks/calibnet/mod.rs:24`                         | `libp2p/filecoin/networks.py:42`                   | implemented | `calibnet -> calibrationnet`.                                         |
...
| Runtime bootstrap filter and conversion    | `node/config/types.go:Libp2p.BootstrapPeers` | `src/libp2p/service.rs:dial_to_bootstrap_peers_if_needed` | `libp2p/filecoin/bootstrap.py:48`, `:79`, `:123`   | implemented | Transport filter + DNS-to-ip4/tcp conversion in DX layer.             |
...
| Mesh defaults (`D`, `Dlo`, `Dhi`, history) | `node/modules/lp2p/pubsub.go:24`             | N/A (Forest does not mirror all knobs identically)        | `libp2p/filecoin/pubsub.py:89`, `:103`, `:106`     | implemented | `8/6/12`, history `10`, initial direct-connect delay `30`.            |
...
| Full topic scoring parity                  | `node/modules/lp2p/pubsub.go:92`             | `src/libp2p/gossip_params.rs:17`                          | `libp2p/filecoin/pubsub.py:34`, `:64`              | partial     | See protocol support matrix for the prioritized scoring/gating gaps.  |
| Message ID function                        | `node/modules/lp2p/pubsub.go:429`            | `src/libp2p/behaviour.rs:79`                              | `libp2p/filecoin/constants.py:38`                  | implemented | Blake2b-256 over message payload.                                     |
