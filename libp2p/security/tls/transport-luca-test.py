...
class TLSTransport(ISecureTransport):
    ...
    def create_ssl_context(self, server_side: bool = False) -> ssl.SSLContext:
        ...
        # Use CERT_NONE for both sides since we do our own verification post-handshake
        # This prevents SSLCertVerificationError from being raised during handshake
    ...
    async def secure_inbound(self, conn: IRawConnection) -> ISecureConn:
        ...
        logger.debug("TLSTransport.secure_inbound: Starting TLS handshake (server side)")
        print("[TLS-TRANSPORT] secure_inbound: Starting TLS handshake (server side)", file=sys.stderr)
        # Create SSL context for server
        ssl_context = self.create_ssl_context(server_side=True)
        logger.debug("TLSTransport.secure_inbound: SSL context created")
        print("[TLS-TRANSPORT] SSL context created", file=sys.stderr)

        # Create TLS reader/writer
        tls_reader_writer = TLSReadWriter(
            conn=conn, ssl_context=ssl_context, server_side=True
        )
        logger.debug("TLSTransport.secure_inbound: TLSReadWriter created, starting handshake")
        print("[TLS-TRANSPORT] TLSReadWriter created, starting handshake", file=sys.stderr)

        # Perform handshake
        await tls_reader_writer.handshake()
        logger.debug("TLSTransport.secure_inbound: TLS handshake completed")
        print("[TLS-TRANSPORT] secure_inbound: TLS handshake completed, extracting peer certificate", file=sys.stderr)
    ...
    async def secure_outbound(self, conn: IRawConnection, peer_id: ID) -> ISecureConn:
        ...
        # Perform handshake
        await tls_reader_writer.handshake()
        print("[TLS-TRANSPORT] secure_outbound: TLS handshake completed, extracting peer certificate", file=sys.stderr)

        # Extract peer information
        peer_cert = tls_reader_writer.get_peer_certificate()
        print(f"[TLS-TRANSPORT] secure_outbound: Peer certificate extracted: {peer_cert is not None}", file=sys.stderr)
        if not peer_cert:
        ...
        remote_public_key = self._extract_public_key_from_cert(peer_cert)
        remote_peer_id = ID.from_pubkey(remote_public_key)
        print(f"[TLS-TRANSPORT] secure_outbound: Remote peer ID: {remote_peer_id.to_string()}, Expected: {peer_id.to_string()}", file=sys.stderr)

        if remote_peer_id != peer_id:
            print(f"[TLS-TRANSPORT] secure_outbound: Peer ID mismatch!", file=sys.stderr)
            raise ValueError(
                f"Peer ID mismatch: expected {peer_id} got {remote_peer_id}"
            )
        print("[TLS-TRANSPORT] secure_outbound: Peer ID verified, creating SecureSession", file=sys.stderr)
