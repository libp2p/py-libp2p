...
from libp2p.stream_muxer.mplex.utils import encode_uvarint

from .pb import crypto_pb2, identify_pb2
...
class IdentifyService:
    ...
    async def request_handler(self, stream: INetStream) -> None:
        ...
        observed_addr = unhexlify(
            stream.mplex_conn.secured_conn.get_conn().remote_addr.to_bytes()
        )
        self_maddrs = tuple(
            unhexlify(maddr.to_bytes()) for maddr in self.host.get_addrs()
        )
        ...
        pubkey_pb = crypto_pb2.PublicKey(Data=pubkey_bytes, Type=crypto_pb2.RSA)
