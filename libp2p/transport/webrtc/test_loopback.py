...
from libp2p.security.noise.transport import PROTOCOL_ID
...
async def build_host(name):
    ...
    await host.get_network().listen(Multiaddr(f"/ip4/127.0.0.1/tcp/9095/ws/p2p/{peer_id}"))
    logger.info(f"Host {name} listening on: {host.get_network().listening_addrs()}")
    return host, peer_id
...
async def run_loopback_test():
    ...
    listener = await webrtc_b.create_listener(lambda conn: logger.info("[B] Listener ready"))

    async def act_as_server():
    ...
    async def act_as_client():
        ...
        conn = await webrtc_a.dial(Multiaddr("/webrtc"))
