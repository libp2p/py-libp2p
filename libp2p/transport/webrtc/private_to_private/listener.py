...
from libp2p.abc import IListener, IHost, INetStream
...
from ..gen_certificate import create_webrtc_multiaddr
...
class WebRTCPeerListener(IListener):
    ...
    def __init__(self, transport: object, handler: THandler, host: IHost) -> None:
        ...
        self.relay_protocol: Optional[CircuitV2Protocol] = None  
    ...
    async def _make_initial_reservations(self) -> None:
        ...
        try:
            ...
            if reservation_count > 0:
                ...
                logger.warning("No relay reservations made - WebRTC signaling may be limited")
    ...
    async def _setup_signaling_handler(self) -> None:
        ...
        self.host.set_stream_handler(
            ...
            self._handle_incoming_signaling_stream
        )
    ...
    async def _handle_incoming_signaling_stream(self, stream: INetStream) -> None:
        ...
        try:
            ...
            if connection:
                ...
                logger.info(f"Successfully established WebRTC connection with {peer_id}")
            else:
    ...
    async def _unregister_handlers(self) -> None:
        ...
        try:
            ...
            if hasattr(self.host, 'get_mux') and hasattr(self.host.get_mux(), 'remove_handler'):
    ...
    async def _cleanup_on_error(self) -> None:
        ...
        try:
            ...
            if self.relay_discovery:
                try:
                    # RelayDiscovery may not have a stop method, so use close if available
            ...
            if self.relay_protocol:
                try:
                    # CircuitV2Protocol may not have a stop method, use close if available
            ...
            if self.relay_discovery:
                ...
                for relay_id in relays:
                    try:
                        ...
                        relay_addrs = self.host.get_peerstore().peer_info(relay_id).addrs
                        
                        for relay_addr in relay_addrs:
                            # Create circuit relay WebRTC multiaddr
                            # Format: /ip4/relay-ip/tcp/relay-port/p2p/relay-id/p2p-circuit/webrtc/p2p/our-id
                            circuit_addr = relay_addr.encapsulate(
                                Multiaddr(f"/p2p/{relay_id}/p2p-circuit/webrtc/p2p/{peer_id}")
                            )
                            addrs.append(circuit_addr)
                            
                    except Exception as e:
                        logger.debug(f"Error creating multiaddr for relay {relay_id}: {e}")
