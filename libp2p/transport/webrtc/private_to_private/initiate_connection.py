...
from libp2p.abc import IRawConnection, INetStream
...
async def initiate_connection(
    ...
    try:
        ...
        await _handle_incoming_ice_candidates(signaling_stream, peer_connection, timeout)
...
async def _receive_signaling_message(stream: INetStream, timeout: float) -> Dict[str, Any]:
    ...
    while True:
        try:
            ...
            if message.get("type") == "ice-candidate":
                ...
                logger.warning(f"Unexpected message during ICE exchange: {message.get('type')}")
