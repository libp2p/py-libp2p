...
# Import WebRTC transport module first to register protocols
...
class OptimizedNetworkTest:
    ...
    async def test_mock_signaling(self) -> None:
        ...
        try:
            # Create signal service without network dependencies
    ...
    async def test_stream_muxing_standalone(self) -> None:
        ...
        try:
            # Create minimal WebRTC setup for stream testing
            ...
            # Verify stream ID assignment
            assert stream_1.stream_id == 1  # First odd number
            assert stream_2.stream_id == 3  # Second odd number  
            assert stream_3.stream_id == 5  # Third odd number
            
            print(f"    Stream IDs: {stream_1.stream_id}, {stream_2.stream_id}, {stream_3.stream_id}")
            ...
            for stream in streams:
                ...
                assert stream.muxed_conn.peer_id == test_peer_id, "Should reference correct peer"
    ...
    def test_protocol_validation(self) -> None:
        ...
        try:
            ...
            print(f"    Protocol codes: WebRTC={hex(CODEC_WEBRTC)}, WebRTC-Direct={hex(CODEC_WEBRTC_DIRECT)}")
            
            # Test multiaddr parsing (should work without network) with valid ED25519 data
            key_pair = create_new_key_pair()
            valid_peer_id = generate_peer_id_from(key_pair)
    ...
    async def test_certificate_standalone(self) -> None:
        ...
        try:
            ...
            assert imported_cert.certhash == cert.certhash, "Import should match"
            
            print("    PEM export/import working")
            ...
            # Test integration with ED25519 peer ID
            key_pair = create_new_key_pair()
            peer_id = generate_peer_id_from(key_pair)
            
            # Create multiaddr with certificate and ED25519 peer ID
            test_maddr = f"/ip4/127.0.0.1/udp/9000/webrtc-direct/certhash/{cert.certhash}/p2p/{peer_id}"
            parsed_maddr = Multiaddr(test_maddr)
            
            print(f"    Integrated multiaddr: {test_maddr}")
