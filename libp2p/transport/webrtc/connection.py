...
class WebRTCStream(INetStream):
    ...
    def __init__(self, stream_id: int, connection: 'WebRTCRawConnection'):
        ...
        self._closed = False
        self.protocol: Optional[TProtocol] = None
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            logger.warning(f"Error closing stream {self.stream_id} receive channel: {e}")
...
class WebRTCRawConnection(IRawConnection):
    ...
    def __init__(self, peer_id: ID, peer_connection: RTCPeerConnection, 
                 data_channel: RTCDataChannel, is_initiator: bool = True):
        ...
        self._next_stream_id = 1 if is_initiator else 2  # Odd for initiator, even for responder
        ...
        try:
            self._trio_token: Optional[Any] = trio.lowlevel.current_trio_token()
        except RuntimeError:
                    ...
                    if self._trio_token:
                        try:
                            trio.from_thread.run_sync(
                                self.send_channel.send_nowait, data, trio_token=self._trio_token
                            )
            ...
            if msg_type == 'data':
                ...
                if stream and not stream._closed:
                    if self._trio_token:
                        try:
                            trio.from_thread.run_sync(
                                stream.send_channel.send_nowait, data, trio_token=self._trio_token
                            )
                        except trio.WouldBlock:
                            logger.warning(f"Stream {stream_id} message dropped: channel full")
                        except Exception as e:
                            logger.error(f"Error sending to stream {stream_id}: {e}")
                    else:
                        # Fallback: store in a buffer or drop
                        logger.warning(f"Cannot deliver message to stream {stream_id}: no trio token")
    ...
    async def close(self) -> None:
        ...
        try:
            ...
            # Close WebRTC resources using async bridge (best effort)
            try:
                async with self._bridge:
            ...
            # Final catch-all for any unexpected cleanup errors
