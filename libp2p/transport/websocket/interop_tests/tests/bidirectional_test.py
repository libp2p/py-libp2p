...
async def test_bidirectional_communication() -> dict[str, Any]:
    ...
    try:
        ...
        for i, message in enumerate(test_messages, 1):
            try:
                ...
                if response and message in response:
                    ...
                    print(
                        f"Exchange {i}/{len(test_messages)}: "
                        f"Failed - unexpected response"
                    )
        ...
        print(
            f"\nResults: {successful_exchanges}/{len(test_messages)} "
            f"successful exchanges"
        )
        ...
        if successful_exchanges == len(test_messages):
            ...
            print("Bidirectional test partially successful")
