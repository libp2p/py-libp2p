...
def status_to_exception(
    ...
    elif status == Message.E_INVALID_PEER_INFO:
        return InvalidPeerInfoError(message)
    elif status == Message.E_INVALID_TTL:
        return InvalidTTLError(message)
    elif status == Message.E_INVALID_COOKIE:
        return InvalidCookieError(message)
    elif status == Message.E_NOT_AUTHORIZED:
        return NotAuthorizedError(message)
    elif status == Message.E_INTERNAL_ERROR:
        return InternalError(message)
    elif status == Message.E_UNAVAILABLE:
