...
class RendezvousService:
    ...
    def _handle_register(
        ...
        logger.info(f"Registered peer {reg_peer_id} in namespace '{namespace}' with TTL {ttl}s")
    ...
    def _handle_unregister(
        self, 
        peer_id: PeerID, 
        unregister_msg: Message.Unregister
    ) -> None:
        ...
        if namespace in self.registrations:
            ...
            logger.info(f"Unregistered peer {target_peer_id} from namespace '{namespace}'")
    ...
    def _handle_discover(
        ...
        pb_registrations = [
            reg.to_protobuf_register() 
            for reg in slice_registrations
        ]
    ...
    def _cleanup_expired_registrations(self, namespace: str) -> None:
        ...
        expired_peers = [
            peer_id for peer_id, record in self.registrations[namespace].items()
        ...
        if expired_peers:
            logger.debug(f"Cleaned up {len(expired_peers)} expired registrations from '{namespace}'")
