...
def maybe_consume_signed_record(
    msg: HopMessage, host: IHost, peer_id: ID | None = None
) -> bool:
    """
    Attempt to parse and store a signed peer record(Envelope) received
    from the HOP message. If the record is invalid, the peer-id
    does not match, or updating the peerstore fails, the function logs an
    error and returns False.

    Only ``HopMessage`` carries ``senderRecord`` per the spec; STOP
    messages have no such field.

    Parameters
    ----------
    msg : HopMessage
        The protobuf message received during relay communication,
        containing a `senderRecord` field.
    host : IHost
