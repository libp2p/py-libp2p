...
async def main():
    ...
    if not kubo_id_info:
        ...
        logger.error(
            "Please install Kubo (https://docs.ipfs.tech/install/command-line/) and run `ipfs daemon` in another terminal."
        )
        return
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            py_payload = {
                "source": "py-ipfs-lite",
                "message": "Hello Kubo, this is Python!",
            }
            ...
            if resp.status_code == 200:
                ...
                logger.info(
                    f"✅ Kubo successfully fetched the data via Bitswap: {kubo_read_data}"
                )
            else:
            ...
            if resp.status_code == 200:
                ...
                py_read_data_bytes = await peer.get_file(
                    kubo_cid, provider_addr=kubo_local_addr
                )
                py_read_data = py_read_data_bytes.decode("utf-8")

                logger.info(
                    f"✅ Python successfully fetched the file via Bitswap: '{py_read_data}'"
                )
            else:
