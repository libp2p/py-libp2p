        ...
        async with self.host.run(listen_addrs=listen_addrs):
            async with background_trio_service(self.pubsub):
                async with background_trio_service(self.gossipsub):
                    ...
                    logger.info(f"Node {self.node_id} (Gossipsub 2.0, {self.role}) started on port {self.port}")
...
class GossipsubV20Demo:
    ...
    async def start_network(self, duration: int = 30):
        ...
        try:
            async with trio.open_nursery() as nursery:
                ...
                print(f"Protocol: /meshsub/2.0.0")
                print(f"Features: Adaptive gossip, advanced security, P6/P7 scoring")
                print(f"{'='*60}\n")
    ...
    def _print_statistics(self):
        ...
        validator_sent = sum(n.messages_sent for n in self.nodes if n.role == "validator")
        ...
        for node in self.nodes:
            print(f"  {node.node_id} ({node.role}): "
                  ...
                  f"validated={node.messages_validated}, rejected={node.messages_rejected}")
