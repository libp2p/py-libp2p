...
This example demonstrates Gossipsub 1.2 protocol (/meshsub/1.2.0).
Gossipsub 1.2 adds IDONTWANT message filtering to reduce redundant message
transmission, improving efficiency in dense networks.

Features demonstrated:
- All Gossipsub 1.1 features (peer scoring, behavioral penalties)
...
logger = logging.getLogger("gossipsub-v1.2")
...
class GossipsubV12Node:
    """A node running Gossipsub 1.2"""
    ...
    async def start(self):
        """Start the node with Gossipsub 1.2 configuration"""
        ...
        # Configure Gossipsub 1.2 - adds IDONTWANT support
        ...
        self.gossipsub = GossipSub(
            ...
            # No adaptive features - v1.2 doesn't have adaptive gossip
            # No advanced security features - v1.2 has basic security
        )
        ...
        async with self.host.run(listen_addrs=listen_addrs):
            async with background_trio_service(self.pubsub):
                async with background_trio_service(self.gossipsub):
                    ...
                    logger.info(
                        f"Node {self.node_id} (Gossipsub 1.2, {self.role}) started on "
                        f"port {self.port}"
                    )
...
class GossipsubV12Demo:
    """Demo controller for Gossipsub 1.2"""

    def __init__(self):
        self.nodes: list[GossipsubV12Node] = []
    ...
    async def setup_network(self, node_count: int = 5):
        ...
        for i in range(node_count):
            ...
            node = GossipsubV12Node(f"node_{i}", port, role)
            self.nodes.append(node)

        logger.info(f"Created network with {node_count} nodes running Gossipsub 1.2")
    ...
    async def start_network(self, duration: int = 30):
        ...
        try:
            async with trio.open_nursery() as nursery:
                ...
                print(f"\n{'=' * 60}")
                print("GOSSIPSUB 1.2 DEMO")
                print(f"{'=' * 60}")
                print(f"Running for {duration} seconds...")
                print("Protocol: /meshsub/1.2.0")
                print("Features: IDONTWANT filtering, reduced bandwidth usage")
                print(f"{'=' * 60}\n")
    ...
    def _print_statistics(self):
        ...
        malicious_sent = sum(
            n.messages_sent for n in self.nodes if n.role == "malicious"
        )
        ...
        for node in self.nodes:
            print(
                f"  {node.node_id} ({node.role}): sent={node.messages_sent}, "
                f"received={node.messages_received}"
            )

        print(f"\n{'=' * 60}")
        print("Gossipsub 1.2 Features:")
        print("  ✓ All Gossipsub 1.1 features")
        print("  ✓ IDONTWANT message filtering")
        print("  ✓ Reduced bandwidth usage")
        print("  ✓ Improved efficiency in dense networks")
        print("  ✗ No adaptive gossip")
        print("  ✗ No advanced security features (P6, P7)")
        print(f"{'=' * 60}\n")


async def main():
    parser = argparse.ArgumentParser(description="Gossipsub 1.2 Example")
    parser.add_argument(
