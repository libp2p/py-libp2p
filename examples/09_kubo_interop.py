...
async def main():
    ...
    try:
        ...
        try:
            ...
            if res.returncode == 0:
                print("Automatically connected local Kubo daemon to Python peer!")
                print("Fetching CID from Python peer using Kubo...")
                cat_res = await trio.run_process(
                    ["ipfs", "cat", py_cid], capture_stdout=True, capture_stderr=True
                )
