...
async def agent_task(
    agent_id: int, peer: Peer, start_event: trio.Event, total_cids: list
):
    ...
    logger.info(
        f"Agent {agent_id} finished {NODES_PER_AGENT} nodes in {duration:.2f}s ({NODES_PER_AGENT / duration:.1f} nodes/sec)"
    )
...
async def gc_task(peer: Peer, stop_event: trio.Event):
    ...
    while not stop_event.is_set():
        ...
        try:
            ...
            logger.warning(
                f"--- GC complete. Reclaimed {reclaimed.reclaimed_blocks} blocks. ---"
            )
        ...
        async with trio.open_nursery() as root_nursery:
            ...
            async with trio.open_nursery() as agent_nursery:
                for i in range(NUM_AGENTS):
                    agent_nursery.start_soon(
                        agent_task, i, peer, start_event, total_cids
                    )
