...
from libp2p.record.validator import Validator, NamespacedValidator
...
def validate_and_select(validator: NamespacedValidator, new_record: Record, existing_record: Optional[Record] = None) -> Optional[Record]:
    ...
    try:
        ...
        if existing_record:
            ...
            selected_index = validator.select(new_record.key_str, candidates)
...
async def create_and_validate_records(host, key_pair, namespace: str, custom_value: str | None, store: SimpleRecordStore, validator: NamespacedValidator):
    ...
    try:
        ...
        if retrieved_record:
            try:
                ...
                logger.info(f"Public key record details:")
        ...
        custom_key = f"/custom/test-key".encode('utf-8')
        ...
        if retrieved_custom:
            try:
                ...
                logger.info(f"Custom record details:")
        ...
        existing_record = store.get(custom_key)
        selected_record = validate_and_select(validator, invalid_record, existing_record)
...
async def run(port: int | None, namespace: str, custom_value: str | None) -> None:
    ...
    async with host.run(listen_addrs=listen_addrs):
        ...
        try:
            ...
            await create_and_validate_records(host, key_pair, namespace, custom_value, store, namespaced_validator)
...
def main() -> None:
    ...
    modules, including registering a custom validator and storing/retrieving records.
    It validates records before storing, uses select for conflicts, and validates on retrieval.
