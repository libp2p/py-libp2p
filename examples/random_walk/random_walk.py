...
async def configure_dht_logging(dht: KadDHT) -> None:
    ...
    rt_refresh_logger = logging.getLogger(
    ...
    while True:
        try:
            ...
            if len(connected_peers) < 20:
                ...
                logger.info(f"Found {len(compatible_peers)} compatible peers")
                logger.info(f"Total peers checked: {len(list_peers)}")
                ...
                async with trio.open_nursery() as task_nursery:
                    ...
                    async def status_reporter():
                        while True:
                            ...
                            logger.debug(
                                "Status - Connected: %d, Routing table: %d, "
                                "Peerstore: %d",
...
def parse_args():
    ...
    if args.verbose:
        ...
        logging.getLogger("libp2p.discovery.random_walk.random_walk").setLevel(
            logging.DEBUG
        )
        logging.getLogger("libp2p.discovery.random_walk.rt_refresh_manager").setLevel(
            logging.DEBUG
        )
