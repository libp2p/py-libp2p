...
from libp2p.security.tls.autotls.acme import ACMEClient
...
def create_rsa_csr(domain: str, cert_key) -> bytes:
    csr = (
...
async def run(port: int, destination: str, psk: int, transport: str) -> None:
        ...
        try:
            ...
            try:
                ...
                if remote_addr:
                    ...
                    print("[IDENTIFY] Remote address: None (could not determine)")
            except Exception as e:
                print(
                    f"[IDENTIFY] Error getting remote address: {type(e).__name__}: {e}"
                )
                import traceback
    ...
    async with host.run(listen_addrs=listen_addrs), trio.open_nursery() as nursery:
        ...
        if not destination:
            ...
            async def wait_for_dns(txt_name: str, a_name: str, timeout: float = 100.0):
                ...
                while True:
                    ...
                    if txt and a:
                        print("\nDNS RECORDS SET BY AUTO-TLS BROKER")
                        return
                    ...
                    print(f"[DNS] waiting… txt={bool(txt)} a={bool(a)}")
                    await trio.sleep(delay)
                    delay = min(delay * 1.5, 10.0)

            async def acme_notify_dns_ready(
                chall_url: str, acct_url: str, priv_key, nonce
            ):
                payload = {}
                ...
                resp = await client.post(chall_url, headers=headers, json=jws)
            ...
            async def acme_finalize_and_fetch_cert(
                finalize_url: str,
                order_url: str,
                csr_b64: str,
                account_url: str,
                ...
                async with httpx.AsyncClient(http2=False, timeout=10.0) as client:
                    ...
                    finalize_resp = resp.json()
                    print("\n[ACME] finalize response:", finalize_resp)
            ...
            async def acme_fetch_certificate(
                cert_url: str, account_url: str, priv_key
            ) -> str:
                nonce = await get_nonce()
            ...
            try:
                ...
                # Fallback: if no public address found
                    ...
                    for addr in all_addrs:
                        try:
                            ...
                            if ip and ip != "127.0.0.1":
                                public_addr = (
                                    f"/ip4/{ip}/tcp/{port}/p2p/{host.get_id()}"
                                )
                                break
                        except Exception:
                            continue

                # Temporary aws-ec2 public-ip for testing
                ec2_ip = "13.126.88.127"
                public_addr = f"/ip4/{ec2_ip}/tcp/{port}/p2p/{host.get_id()}"

                if public_addr is None:
                    raise RuntimeError(
                        "Could not determine public IP address for broker"
                    )

                print(f"\n[DEBUG] Using public address for broker: {public_addr}")
                public_addrs = [public_addr]

                await http_peer_id_auth(host.get_private_key(), key_auth, public_addrs)

                # Next step: Polling for DNS records
                peer = b36_peer_id
                txt_name = f"_acme-challenge.{peer}.libp2p.direct"
                a_name = f"{ec2_ip.replace('.', '-')}.{peer}.libp2p.direct"

                await wait_for_dns(txt_name, a_name)

                nonce, body = await acme_notify_dns_ready(
                    chall_url, account_url, priv_key, await get_nonce()
                )
                while True:
                    nonce, body = await acme_notify_dns_ready(
                        chall_url, account_url, priv_key, nonce
                    )

                    status = body.get("status")
                    print("[ACME] challenge status:", status)

                    if status == "valid":
                        print("\n\nACME CHALLENGE NOTIFIED: SUCCESSFULLY")
                        break
                ...
                domain = f"*.{b36_peer_id}.libp2p.direct"

                csr_der = create_rsa_csr(domain, cert_key)
                csr_b64 = b64url(csr_der)

                cert_url = await acme_finalize_and_fetch_cert(
                    finalize_url, order_url, csr_b64, account_url, priv_key
                )
                cert_chain = await acme_fetch_certificate(
                    cert_url, account_url, priv_key
                )

                print("\n\n")
                print(cert_chain)

                with open("cert-chain.pem", "rb") as f:
                    pem_data = f.read()

                cert = x509.load_pem_x509_certificates(pem_data)

                leaf = cert[0]
                intermediates = cert[1:]
