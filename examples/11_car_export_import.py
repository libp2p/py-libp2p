...
async def main():
    ...
    print("\n--- Phase 1: Writer Peer ---")
    writer = Peer(Config(reprovide_interval_seconds=-1, blockstore_type="memory"), listen_addrs=["/ip4/127.0.0.1/tcp/0"])
    await writer.start()
    ...
    root = await writer.add_node({"title": "My Book", "chapters": [{"/": leaf_a}, {"/": leaf_b}]}, codec="dag-cbor")
    ...
    reader = Peer(Config(offline=True, reprovide_interval_seconds=-1, blockstore_type="memory"), listen_addrs=["/ip4/127.0.0.1/tcp/0"])
    await reader.start()
