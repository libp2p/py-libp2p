...
async def main():
    ...
    print(f"Connecting Peer B to Peer A using multiaddr: {peer_a_addr}")
    await peer_b.bootstrap([peer_a_addr])
    print("Peers connected successfully.")

    print("Adding current file to Peer A's blockstore...")
    ...
    print("Requesting file by CID from Peer B (without providing Peer A's address)...")
    content = await peer_b.get_file(cid)
    print(f"Peer B fetched {len(content)} bytes without being told who has it.")

    assert len(content) > 0

    print("Closing both peers cleanly...")
    await peer_b.close()
    await peer_a.close()
