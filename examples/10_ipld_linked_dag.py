...
async def main():
    ...
    dataset_cid = await peer.add_node(
        {"name": "wikitext-103", "size_gb": 1.8, "sha256": "abc..."}, codec="dag-cbor"
    )
    print(f"1. Dataset node added: {dataset_cid}")

    checkpoint_cid = await peer.add_node(
        {"epoch": 42, "loss": 0.031, "dataset": {"/": dataset_cid}}, codec="dag-cbor"
    )
    print(f"2. Checkpoint node added: {checkpoint_cid}")

    model_cid = await peer.add_node(
        {"arch": "transformer", "params": "7B", "checkpoint": {"/": checkpoint_cid}},
        codec="dag-cbor",
    )
    print(f"3. Model node added: {model_cid}")

    agent_cid = await peer.add_node(
        {"id": "summarizer-v2", "model": {"/": model_cid}, "version": "2.1.0"},
        codec="dag-cbor",
    )
