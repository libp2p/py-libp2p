...
class QUICTransport(ITransport):
    ...
    async def dial(
        ...
        try:
            ...
            base_config = self._quic_configs.get(config_key)
            if not base_config:
                raise QUICDialError(f"Unsupported QUIC version: {quic_version}")

            config = create_client_config_from_base(
                base_config, self._security_manager, self._config
            )

            if self._config.enable_qlog:
