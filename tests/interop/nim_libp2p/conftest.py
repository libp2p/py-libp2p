...
def nim_echo_binary():
    """Get nim echo server binary, building it if necessary."""
    # Fast check - if binary exists, return immediately
    if check_nim_binary_built():
        current_dir = Path(__file__).parent
        return current_dir / "nim_echo_server"

    # Binary doesn't exist, check if we can build it
    if not check_nim_available():
        pytest.skip(
            "Nim compiler not found. Install nim first:\n"
            "curl -sSf https://nim-lang.org/choosenim/init.sh | sh"
        )

    # Run setup with locking
    try:
        run_nim_setup_with_lock()
    except subprocess.TimeoutExpired:
        pytest.skip("nim-libp2p setup timed out")
    except Exception as e:
        pytest.skip(f"nim-libp2p setup failed: {e}")
    ...
    if not binary_path.exists():
        pytest.skip("nim_echo_server binary not found after setup")
