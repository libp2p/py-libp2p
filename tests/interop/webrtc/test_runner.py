...
class WebRTCTestRunner:
    ...
    async def clear_state(self):
        ...
        try:
            ...
            keys = await self.redis_client.keys("interop:webrtc:*")
            if keys:
    ...
    async def wait_for_key(self, key: str, timeout: int = None) -> bool:
    ...
    async def test_py_listener_go_dialer(self):
        ...
        try:
            ...
            if not await self.wait_for_key("interop:webrtc:listener:ready", self.timeout):
            ...
            await asyncio.sleep(2)  # Give dialer time to start and detect Python listener
            
            # Step 4: Wait for connection (signaling layer)
            logger.info("Step 4: Waiting for connection signal...")
            if not await self.wait_for_key("interop:webrtc:dialer:connected", self.timeout):
    ...
    async def test_go_listener_py_dialer(self):
        ...
        try:
            ...
            if not await self.wait_for_key("interop:webrtc:go:listener:ready", self.timeout):
            ...
            go_multiaddr = await self.get_redis_value("interop:webrtc:go:listener:multiaddr")
            if not go_multiaddr:
            ...
            await self.redis_client.set("interop:webrtc:py:connect", go_multiaddr)
            
            # Step 4: Wait for connection
            logger.info("Step 4: Waiting for Python dialer to connect...")
            if not await self.wait_for_key("interop:webrtc:py:dialer:connected", self.timeout):
                logger.error("FAILED: Python dialer never connected")
                return False
            
            # Step 5: Wait for ping
            logger.info("Step 5: Waiting for ping/pong test...")
            if not await self.wait_for_key("interop:webrtc:py:ping:success", self.timeout):
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            self.test_results['py-listener-go-dialer'] = await self.test_py_listener_go_dialer()
            
            await asyncio.sleep(2)
            await self.clear_state()
            
            # Test 2: Go listener, Python dialer
            self.test_results['go-listener-py-dialer'] = await self.test_go_listener_py_dialer()
