        ...
        try:
            ...
            while time.time() - start_time < timeout:
                if self.process.poll() is not None:
                    ...
                    raise RuntimeError(f"Go node exited early. stdout: {stdout}, stderr: {stderr}")
                ...
                if line:
                    ...
                    if line.startswith("ADDR:"):
                        ...
                        logger.info(f"Go-libp2p node ready at {self.addr} with peer ID {self.peer_id}")
                        return
...
async def test_floodsub_basic_functionality():
    ...
    private_key1 = Secp256k1PrivateKey.generate()
    private_key2 = Secp256k1PrivateKey.generate()
