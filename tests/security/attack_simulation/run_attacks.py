...
async def run_invalid_block_attack():
    ...
    try:
        ...
        pre_finality = results.get("pre_finality_metrics", {})
        lc_rate = pre_finality.get("light_client_acceptance_rate", 0)
        fn_rate = pre_finality.get("full_node_acceptance_rate", 0)
        vuln_gap = pre_finality.get("vulnerability_gap", 0)

        print(f"   - Light client acceptance rate: {lc_rate:.2%}")
        print(f"   - Full node acceptance rate: {fn_rate:.2%}")
        print(f"   - Vulnerability gap: {vuln_gap:.2%}")
...
async def run_bootnode_poisoning_attack():
    ...
    try:
        ...
        print(
            f"   - Isolation rate: "
            f"{results.get('isolation_metrics', {}).get('isolation_rate', 0):.2%}"
        )
        print(
            f"   - Recovery rate: "
            f"{results.get('recovery_metrics', {}).get('recovery_rate', 0):.2%}"
        )
        perm_iso = results.get("attack_persistence", {}).get(
            "permanent_isolation_rate", 0
        )
        print(f"   - Permanent isolation: {perm_iso:.2%}")
...
async def run_finality_stall_attack():
    ...
    try:
        ...
        print(f"   - Attack type: {results.get('attack_type', 'N/A')}")
        print(
            f"   - Memory exhaustion rate: "
            f"{results.get('memory_metrics', {}).get('exhaustion_rate', 0):.2%}"
        )
        print(
            f"   - Peak memory usage: "
            f"{results.get('memory_metrics', {}).get('peak_memory_mb', 0):.1f} MB"
        )
        timeout_rate = results.get("detection_metrics", {}).get(
            "timeout_detection_rate", 0
        )
        print(f"   - Timeout detection rate: {timeout_rate:.2%}")
...
async def run_long_range_fork_attack():
    ...
    try:
        ...
        print(f"   - Attack type: {results.get('attack_type', 'N/A')}")
        print(
            f"   - Fork replay success rate: "
            f"{results.get('fork_metrics', {}).get('replay_success_rate', 0):.2%}"
        )
        print(
            f"   - Fork detection rate: "
            f"{results.get('detection_metrics', {}).get('detection_rate', 0):.2%}"
        )
        print(
            f"   - Resync success rate: "
            f"{results.get('resync_metrics', {}).get('success_rate', 0):.2%}"
        )
...
async def main():
    ...
    print("=" * 60)
    print(
        "Running comprehensive attack simulations inspired by "
        "Polkadot/Smoldot security research"
    )
    print()
    ...
    print(
        "💡 Check the detailed results above for security insights and recommendations."
    )
