...
from .real_network_builder import RealNetworkBuilder, RealMaliciousPeer
...
class RealEclipseScenario(EclipseScenario):
    ...
    def __init__(self, honest_hosts, honest_dhts, malicious_peers):
        ...
        # Track honest peer IDs for compatibility
        self.honest_peers = [host.get_id().to_string() for host in honest_hosts]
    
    async def execute_real_attack(self, attack_duration: float = 30.0):
        """Execute a real eclipse attack against actual libp2p network"""
        
        print(f"🎯 Starting Real Eclipse Attack Simulation")
        print(f"📊 Network: {len(self.honest_hosts)} honest nodes, {len(self.malicious_peers)} malicious nodes")
    ...
    def _print_attack_summary(self, results):
        ...
        during_contamination = results['during_attack']['contamination']['overall_contamination_rate']
        after_contamination = results['after_attack']['contamination']['overall_contamination_rate']
...
async def test_real_eclipse_attack_simulation():
    ...
    honest_hosts, honest_dhts, malicious_peers = await builder.create_real_eclipse_test_network(
...
async def test_real_metrics_collection():
    ...
    contamination_results = await metrics.measure_routing_table_contamination(honest_dhts, malicious_ids)
