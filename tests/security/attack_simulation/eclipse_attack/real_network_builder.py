...
class RealMaliciousPeer(MaliciousPeer):
    ...
    async def poison_real_dht_entries(self, target_dht: KadDHT, fake_peer_info: PeerInfo):
...
class RealNetworkBuilder(AttackNetworkBuilder):
    ...
    async def _connect_nodes(self, hosts: List[IHost]):
        ...
        for i in range(len(hosts)):
            for j in range(i + 1, min(i + 3, len(hosts))):  # Each node connects to 2 neighbors
                try:
    ...
    async def setup_real_attack_scenario(
        ...
        return await self.create_real_eclipse_test_network(honest_count, malicious_count)