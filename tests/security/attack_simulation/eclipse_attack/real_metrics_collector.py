...
class RealAttackMetrics(AttackMetrics):
    ...
    async def measure_real_lookup_performance(
        self, 
        dhts: List[KadDHT], 
        num_test_keys: int = 10,
        lookups_per_key: int = 5
    ) -> Dict[str, float]:
    ...
    async def measure_network_connectivity(
        self, 
        hosts: List[IHost]
    ) -> Dict[str, Any]:
    ...
    async def measure_routing_table_contamination(
        ...
        for dht in honest_dhts:
            ...
            dht_contamination = malicious_count / len(routing_table_peers) if routing_table_peers else 0
            contamination_per_dht.append(dht_contamination)
        
        overall_contamination = total_contamination / total_entries if total_entries > 0 else 0
        
        return {
            'overall_contamination_rate': overall_contamination,
            'average_contamination_per_dht': sum(contamination_per_dht) / len(contamination_per_dht) if contamination_per_dht else 0,
    ...
    async def measure_complete_attack_cycle(
        ...
        results['before_attack']['lookup_performance'] = await self.measure_real_lookup_performance(honest_dhts)
        results['before_attack']['connectivity'] = await self.measure_network_connectivity(honest_hosts)
        results['before_attack']['contamination'] = await self.measure_routing_table_contamination(
        ...
        async with trio.open_nursery() as nursery:
            for malicious_peer in malicious_peers:
                for honest_dht in honest_dhts:
                    ...
                    fake_peer_info = PeerInfo(malicious_peer.host.get_id(), malicious_peer.host.get_addrs())
                    nursery.start_soon(malicious_peer.poison_real_dht_entries, honest_dht, fake_peer_info)
        ...
        results['during_attack']['lookup_performance'] = await self.measure_real_lookup_performance(honest_dhts)
        results['during_attack']['connectivity'] = await self.measure_network_connectivity(honest_hosts)
        results['during_attack']['contamination'] = await self.measure_routing_table_contamination(
            honest_dhts, [mp.peer_id for mp in malicious_peers]
        )
        ...
        recovery_time = 60.0  # 1 minute recovery
        await trio.sleep(recovery_time)
        
        # Phase 5: Measure recovery
        results['after_attack']['lookup_performance'] = await self.measure_real_lookup_performance(honest_dhts)
        results['after_attack']['connectivity'] = await self.measure_network_connectivity(honest_hosts)
        results['after_attack']['contamination'] = await self.measure_routing_table_contamination(
            honest_dhts, [mp.peer_id for mp in malicious_peers]
        )
    ...
    def _calculate_recovery_metrics(self, results: Dict[str, Any]) -> Dict[str, float]:
        ...
        attack_effectiveness = (before_success - during_success) / before_success if before_success > 0 else 0
        recovery_effectiveness = (after_success - during_success) / (before_success - during_success) if (before_success - during_success) > 0 else 0
        
        before_connectivity = results['before_attack']['connectivity']['connectivity_ratio']
        during_connectivity = results['during_attack']['connectivity']['connectivity_ratio']
        after_connectivity = results['after_attack']['connectivity']['connectivity_ratio']
        
        connectivity_impact = (before_connectivity - during_connectivity) / before_connectivity if before_connectivity > 0 else 0
        connectivity_recovery = (after_connectivity - during_connectivity) / (before_connectivity - during_connectivity) if (before_connectivity - during_connectivity) > 0 else 0
        ...
        return {
            ...
            'overall_network_resilience': (recovery_effectiveness + connectivity_recovery) / 2
        }