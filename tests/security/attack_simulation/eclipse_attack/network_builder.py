...
class AttackNetworkBuilder:
    ...
    async def create_eclipse_test_network(
        self, honest_nodes: int = 10, malicious_nodes: int = 3, attack_intensity: float = 0.5
    ) -> tuple[list[str], list[MaliciousPeer]]:
        honest = [f"honest_{i}" for i in range(honest_nodes)]
        malicious = [
            MaliciousPeer(f"mal_{i}", "eclipse", attack_intensity) for i in range(malicious_nodes)
        ]
