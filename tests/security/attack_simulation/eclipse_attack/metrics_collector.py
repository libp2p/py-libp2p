...
class AttackMetrics:
    ...
    def calculate_metrics(self, honest_peers: List[str], malicious_peers: List[Any], attack_intensity: float):
        """Calculate attack metrics based on network state and attack parameters"""
        self.honest_peers = honest_peers
        self.malicious_peers = [str(peer.peer_id) if hasattr(peer, 'peer_id') else str(peer) for peer in malicious_peers]
        ...
        self.routing_table_poisoning_attempts = int(len(malicious_peers) * attack_intensity * 5)
        
        # Network isolation is more likely with higher intensity and more malicious peers
        isolation_probability = min(attack_intensity * len(malicious_peers) / len(honest_peers), 0.9)
        self.network_isolation_achieved = attack_intensity > 0.5 and len(malicious_peers) >= len(honest_peers) * 0.3
        
        if self.network_isolation_achieved:
            self.isolation_duration = attack_intensity * 10.0  # seconds
    ...
    def generate_attack_report(self) -> Dict[str, Any]:
        ...
        avg_connectivity = sum(self.network_connectivity) / len(self.network_connectivity) if self.network_connectivity else 0
        avg_lookup_success = sum(self.lookup_success_rate) / len(self.lookup_success_rate) if self.lookup_success_rate else 0
        avg_message_delivery = sum(self.message_delivery_rate) / len(self.message_delivery_rate) if self.message_delivery_rate else 0
        
        network_resilience_score = (avg_connectivity * 0.4 + avg_lookup_success * 0.3 + avg_message_delivery * 0.3) * 100
        
        # Generate mitigation recommendations based on attack metrics
        mitigation_recommendations = []
        
        if avg_connectivity < 0.7:
            mitigation_recommendations.append("Implement redundant peer connections to improve network connectivity")
        if avg_lookup_success < 0.8:
            mitigation_recommendations.append("Deploy DHT redundancy mechanisms to improve lookup success rates")
        if self.dht_poisoning_rate > 0.3:
            mitigation_recommendations.append("Enable DHT entry validation and signature verification")
        if self.peer_table_flooding_rate > 0.5:
            mitigation_recommendations.append("Implement rate limiting for peer table updates")
        if self.routing_disruption_level > 0.6:
            mitigation_recommendations.append("Deploy alternative routing protocols for resilience")
        if self.affected_nodes_percentage > 60:
            mitigation_recommendations.append("Increase network size and diversity to reduce attack impact")
        if self.recovery_time > 20:
            mitigation_recommendations.append("Implement faster recovery mechanisms and backup connections")
        
        if not mitigation_recommendations:
            mitigation_recommendations.append("Current network resilience is adequate for the given attack scenario")
        ...
        return {
            ...
            "peer_metrics": {
                ...
                "average_peer_table_contamination": sum(self.peer_table_contamination) / len(self.peer_table_contamination) if self.peer_table_contamination else 0,
            ...
            "resource_metrics": {
                ...
                "average_cpu_utilization": sum(self.cpu_utilization) / len(self.cpu_utilization) if self.cpu_utilization else 0,
                "average_bandwidth_consumption": sum(self.bandwidth_consumption) / len(self.bandwidth_consumption) if self.bandwidth_consumption else 0,
