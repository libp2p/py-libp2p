...
class AttackAnalysis:
    ...
    def assess_vulnerabilities(self, metrics: AttackMetrics) -> Dict[str, Any]:
        ...
        vulnerabilities = {
            "lookup_success_degradation": (
                ...
                if len(metrics.lookup_success_rate) >= 2 else 0
            ),
            "max_contamination": max(metrics.peer_table_contamination) if metrics.peer_table_contamination else 0,
            "connectivity_impact": (
                metrics.network_connectivity[0] - metrics.network_connectivity[1]
                if len(metrics.network_connectivity) >= 2 else 0
            ),
            "resource_stress": (
                max(metrics.cpu_utilization) / metrics.cpu_utilization[0]
                if metrics.cpu_utilization else 1.0
            ),
    ...
    def suggest_mitigations(self, metrics: AttackMetrics) -> List[str]:
        ...
        if metrics.attack_persistence > 0.8:
            recommendations.append("Implement persistent attack monitoring and automated response")
    ...
    def calculate_resilience(self, metrics: AttackMetrics) -> float:
        ...
        contamination_penalty = max(metrics.peer_table_contamination) * 30 if metrics.peer_table_contamination else 0
        ...
        resource_penalty = (
            ...
            if metrics.cpu_utilization else 0
        )

        resilience_score = base_score - lookup_penalty - contamination_penalty - connectivity_penalty - resource_penalty
    ...
    def assess_overall_risk(self, metrics: AttackMetrics) -> Dict[str, Any]:
        ...
        if resilience_score >= 80:
            ...
            risk_level = "Medium"
            risk_description = "Network has moderate vulnerabilities that should be addressed"
    ...
    def _assess_vulnerability_severity(self, metrics: AttackMetrics) -> str:
        ...
        if len(metrics.lookup_success_rate) >= 2:
            degradation = metrics.lookup_success_rate[0] - metrics.lookup_success_rate[1]
    ...
    def _identify_weak_points(self, metrics: AttackMetrics) -> List[str]:
        ...
        if len(metrics.lookup_success_rate) >= 2 and metrics.lookup_success_rate[1] < 0.5:
        ...
        if len(metrics.network_connectivity) >= 2 and metrics.network_connectivity[1] < 0.5:
    ...
    def _evaluate_defense_effectiveness(self, metrics: AttackMetrics) -> Dict[str, Any]:
        ...
        effectiveness = {
            ...
            "mitigation_strength": "Strong" if metrics.mitigation_effectiveness > 0.8 else "Weak",
    ...
    def _identify_critical_findings(self, metrics: AttackMetrics) -> List[str]:
        ...
        if len(metrics.lookup_success_rate) >= 2 and metrics.lookup_success_rate[1] < 0.3:
