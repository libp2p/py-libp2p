...
class FloodingMaliciousPeer:
    ...
    def __init__(self, peer_id: str, flood_type: str, intensity: float):
        ...
        self.flood_start_time: Optional[float] = None
        self.flood_end_time: Optional[float] = None

    async def initiate_pubsub_flood(self, target_topics: List[str], duration: float = 30.0):
        ...
        async with trio.open_nursery() as nursery:
            for topic in target_topics:
                nursery.start_soon(self._flood_topic, topic, messages_per_second, duration)
    ...
    async def initiate_connection_flood(self, target_peers: List[str], duration: float = 30.0):
        ...
        async with trio.open_nursery() as nursery:
            for target in target_peers:
                nursery.start_soon(self._flood_connections, target, connections_per_second, duration)
    ...
    async def initiate_message_flood(self, target_peers: List[str], duration: float = 30.0):
        ...
        async with trio.open_nursery() as nursery:
            for target in target_peers:
                nursery.start_soon(self._flood_messages, target, messages_per_second, duration)
...
class FloodingAttackScenario:
    ...
    def __init__(self, honest_peers: List[str], flooding_attackers: List[FloodingMaliciousPeer]):
        self.honest_peers = honest_peers
        self.flooding_attackers = flooding_attackers
        self.metrics = AttackMetrics()

    async def execute_flooding_attack(self, attack_duration: float = 30.0) -> Dict[str, Any]:
        ...
        async with trio.open_nursery() as nursery:
            for attacker in self.flooding_attackers:
                if attacker.flood_type == "pubsub":
                    nursery.start_soon(attacker.initiate_pubsub_flood,
                                     [f"topic_{i}" for i in range(5)], attack_duration)
                elif attacker.flood_type == "connection":
                    nursery.start_soon(attacker.initiate_connection_flood,
                                     self.honest_peers, attack_duration)
                elif attacker.flood_type == "message":
                    nursery.start_soon(attacker.initiate_message_flood,
                                     self.honest_peers, attack_duration)
        ...
        self._calculate_flooding_metrics(total_messages, total_connections, attack_duration)
    ...
    def _calculate_flooding_metrics(self, total_messages: int, total_connections: int, duration: float):
        ...
        self.metrics.peer_table_contamination = [0.0, min(flood_impact * 0.3, 0.5), flood_impact * 0.2]
        self.metrics.network_connectivity = [1.0, max(1.0 - flood_impact * 0.6, 0.3), 0.85]
        self.metrics.message_delivery_rate = [0.98, max(0.98 - flood_impact * 0.8, 0.1), 0.90]

        # Flooding attack metrics
        self.metrics.time_to_partitioning = max(10, 60 - msg_rate / 10)  # Faster with higher rates
        self.metrics.affected_nodes_percentage = min(flood_impact * 100, 90)
        ...
        self.metrics.memory_usage = [base_memory, base_memory + memory_impact, base_memory * 1.1]
        self.metrics.cpu_utilization = [base_cpu, base_cpu + cpu_impact, base_cpu * 1.2]
        self.metrics.bandwidth_consumption = [base_bandwidth, base_bandwidth + bandwidth_impact, base_bandwidth * 1.3]

        # Recovery metrics
        self.metrics.recovery_time = flood_impact * 30 + 10  # Recovery time based on impact
        self.metrics.detection_time = 5.0  # Flooding is usually quickly detected
        self.metrics.mitigation_effectiveness = 0.8  # Good mitigation possible with rate limiting
