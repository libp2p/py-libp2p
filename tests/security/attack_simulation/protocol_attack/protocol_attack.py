...
class ProtocolExploitAttacker:
    ...
    def __init__(self, peer_id: str, exploit_type: str, intensity: float):
        ...
        self.exploit_type = exploit_type  # "malformed_msg", "protocol_violation", "handshake_exploit"
        self.intensity = intensity
        self.exploits_attempted: List[Dict[str, Any]] = []
        self.successful_exploits: List[Dict[str, Any]] = []
        self.victims_affected: List[str] = []

    async def execute_malformed_message_attack(self, target_peers: List[str], duration: float = 30.0):
        ...
        async with trio.open_nursery() as nursery:
            for target in target_peers:
                nursery.start_soon(self._send_malformed_messages,
                                  target, exploits_per_second, duration)

    async def _send_malformed_messages(self, target: str, exploit_rate: int, duration: float):
        ...
        while trio.current_time() < end_time:
            for i in range(exploit_rate):
                ...
                if trio.current_time() % 2 < self.intensity:  # Success based on intensity
    ...
    async def execute_protocol_violation_attack(self, target_peers: List[str], duration: float = 30.0):
        ...
        async with trio.open_nursery() as nursery:
            for target in target_peers:
                nursery.start_soon(self._send_protocol_violations,
                                  target, violations_per_second, duration)

    async def _send_protocol_violations(self, target: str, violation_rate: int, duration: float):
    ...
    async def execute_handshake_exploit(self, target_peers: List[str], duration: float = 30.0):
        """Exploit handshake protocol weaknesses"""
        handshakes_per_second = int(3 * self.intensity)  # 0-3 handshake exploits/sec

        async with trio.open_nursery() as nursery:
            for target in target_peers:
                nursery.start_soon(self._exploit_handshakes,
                                  target, handshakes_per_second, duration)

    async def _exploit_handshakes(self, target: str, handshake_rate: int, duration: float):
...
class ProtocolAttackScenario:
    ...
    def __init__(self, honest_peers: List[str], protocol_attackers: List[ProtocolExploitAttacker]):
        self.honest_peers = honest_peers
        self.protocol_attackers = protocol_attackers
        self.metrics = AttackMetrics()

    async def execute_protocol_attack(self, attack_duration: float = 30.0) -> Dict[str, Any]:
        ...
        async with trio.open_nursery() as nursery:
            for attacker in self.protocol_attackers:
                if attacker.exploit_type == "malformed_msg":
                    nursery.start_soon(attacker.execute_malformed_message_attack,
                                     self.honest_peers, attack_duration)
                elif attacker.exploit_type == "protocol_violation":
                    nursery.start_soon(attacker.execute_protocol_violation_attack,
                                     self.honest_peers, attack_duration)
                elif attacker.exploit_type == "handshake_exploit":
                    nursery.start_soon(attacker.execute_handshake_exploit,
                                     self.honest_peers, attack_duration)
        ...
        self._calculate_protocol_metrics(total_exploits, successful_exploits, len(affected_victims), attack_duration)
    ...
    def _calculate_protocol_metrics(self, total_exploits: int, successful_exploits: int, victims_affected: int, duration: float):
        """Calculate metrics specific to protocol attacks"""
        success_rate = successful_exploits / total_exploits if total_exploits > 0 else 0
        victim_ratio = victims_affected / len(self.honest_peers) if self.honest_peers else 0
        ...
        self.metrics.lookup_success_rate = [base_success, during_attack, base_success * 0.98]
        self.metrics.peer_table_contamination = [0.0, victim_ratio * 0.1, victim_ratio * 0.05]
        self.metrics.network_connectivity = [1.0, max(1.0 - protocol_impact * 0.7, 0.5), 0.95]
        self.metrics.message_delivery_rate = [0.98, max(0.98 - protocol_impact * 0.5, 0.6), 0.97]
        ...
        self.metrics.attack_persistence = success_rate * 0.8  # Can be persistent if exploits work

        # Resource impact - protocol exploits use fewer resources but cause targeted damage
        base_memory = 100
        base_cpu = 10
        base_bandwidth = 50
        ...
        self.metrics.memory_usage = [base_memory, base_memory + memory_impact, base_memory * 1.02]
        self.metrics.cpu_utilization = [base_cpu, base_cpu + cpu_impact, base_cpu * 1.05]
        self.metrics.bandwidth_consumption = [base_bandwidth, base_bandwidth + bandwidth_impact, base_bandwidth * 1.1]

        # Recovery metrics
        self.metrics.recovery_time = protocol_impact * 45 + 15  # Protocol fixes take time
        self.metrics.detection_time = 10.0  # Protocol issues are detectable
        self.metrics.mitigation_effectiveness = 0.95  # Very effective with protocol updates
