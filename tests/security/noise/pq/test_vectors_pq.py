...
Loads the 5 vendored vectors (tests/fixtures/pqc-test-vectors.json) and
reproduces each handshake with the exact same fixed seeds. Asserts
byte-exact equality of:
...
wire-compatible. Vectors were generated by js-libp2p-noise and committed
here so they run in CI on every clean checkout.
