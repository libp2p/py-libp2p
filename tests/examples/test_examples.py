...
async def pubsub_demo(host_a, host_b):
    # Initialize GossipSub with appropriate parameters
    ...
    async def handle_subscription_b(subscription):
        ...
        logger.debug("Host B received: %s", received_by_b)
        b_received.set()
...
async def test_protocols(test, security_protocol):
    logger.debug("Testing security protocol: %s", security_protocol)
