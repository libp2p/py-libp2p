...
async def test_large_scale_fanout():
    ...
    async with PubsubFactory.create_batch_with_gossipsub(
        ...
        async with trio.open_nursery() as nursery:
            ...
            peers_received = sum(
                1 for msgs in received_messages 
...
async def test_simulated_partition():
    """Test network partition using topic isolation instead of actual network partition."""
    ...
    async with PubsubFactory.create_batch_with_gossipsub(
        ...
        async with trio.open_nursery() as nursery:
            ...
            for i in range(2):
                assert any(msg.data == message_group1 for msg in received_messages[topic_group1][i])
            ...
            for i in range(2, 4):
                assert any(msg.data == message_group2 for msg in received_messages[topic_group2][i])
            ...
            for i in range(len(pubsubs)):
                assert any(msg.data == message_common for msg in received_messages[common_topic][i])
...
async def test_mesh_stability():
    ...
    async with PubsubFactory.create_batch_with_gossipsub(
        ...
        # Record initial mesh sizes
        initial_mesh_sizes = [len(gsub.mesh[topic]) for gsub in gsubs]
        ...
        async with trio.open_nursery() as nursery:
            ...
            assert peers_received >= 3  # At least 75% of peers should receive the message
