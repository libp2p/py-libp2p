...
try:
    ...
    try:
        ...
        ExceptionGroup = Exception  # type: ignore[assignment, misc]
...
async def test_wss_listen_without_tls_config():
    ...
    try:
        ...
        # Check for ExceptionGroup (has exceptions attribute)
        # Use hasattr to check for ExceptionGroup since it might be the fallback
        if (
            ...
            if hasattr(e, "exceptions"):
                ...
                # ExceptionGroup.exceptions exists when not the fallback Exception
                exceptions = getattr(e, "exceptions", [])  # type: ignore[attr-defined]
                ...
                if not found_tls_error:
                    pytest.fail(
                        f"Expected TLS configuration error in ExceptionGroup, "
                        f"got: {exceptions}"
                    )
        else:
            pytest.fail(
