...
@pytest.mark.trio
async def test_event_processing_loop_active_does_not_busy_spin():
    """
    A connection that always has events available must pace its event loop.

    Regression: the loop used ``trio.sleep(0)`` whenever events were
    processed, so an active connection (in-flight handshake, flow-control
    updates, acks) spun the trio event loop at full speed — with dozens of
    such connections the core saturated and peers got starved/dropped.
    The loop must now sleep at least ``_ACTIVE_POLL_INTERVAL`` per
    iteration instead of yielding at zero cost.
    """
    mock_quic = Mock()
    ...
    transport = Mock()
    transport._config = QUICTransportConfig()
    ...
    conn._socket = None  # _transmit() becomes a no-op

    from aioquic.quic.events import PingAcknowledged

    event = PingAcknowledged(uid=1)
    calls = {"n": 0}
    ...
    mock_quic.next_event.side_effect = always_events

    with trio.move_on_after(0.6):
        async with trio.open_nursery() as nursery:
...
@pytest.mark.trio
async def test_event_processing_loop_idle_sleeps():
    """
    An idle connection (no events, no timers) must not wake the loop fast.

    Regression: the loop polled every 1-10ms regardless of activity, so a
    node with hundreds of idle QUIC connections churned the trio timer heap
    and saturated the CPU.  The loop must now sleep the full idle interval
    (``_IDLE_POLL_INTERVAL`` = 50ms) when nothing is pending.
    """
    mock_quic = Mock()
    mock_quic.next_event.return_value = None
    mock_quic.datagrams_to_send.return_value = []
    ...
    def counting_next_event():
        calls["n"] += 1
        return None

    mock_quic.next_event.side_effect = counting_next_event
