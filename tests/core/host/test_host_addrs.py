...
async def test_host_addrs_separation():
    ...
    # Mock get_peer_id on network (BasicHost delegates get_id to network.get_peer_id usually? 
    # No, BasicHost.get_id() calls self._network.get_peer_id())
    ...
    assert addr_str == f"{transport_addr_str}{expected_suffix_p2p}" or \
           addr_str == f"{transport_addr_str}{expected_suffix_ipfs}"
