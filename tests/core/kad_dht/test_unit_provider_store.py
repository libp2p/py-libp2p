...
class TestProviderStore:
    ...
    def test_get_providers_expired_records(self):
        ...
        store.providers[key][str(peer_id2)] = ProviderRecord(provider2, expired_timestamp)
    ...
    def test_get_providers_cleanup_empty_key(self):
        ...
        expired_timestamp = time.time() - PROVIDER_RECORD_EXPIRATION_INTERVAL - 1
        store.providers[key] = {str(peer_id): ProviderRecord(provider, expired_timestamp)}
    ...
    def test_cleanup_expired_remove_empty_keys(self):
        ...
        provider2 = PeerInfo(peer_id2, [])
        expired_timestamp = time.time() - PROVIDER_RECORD_EXPIRATION_INTERVAL - 1
        store.providers[key2] = {str(peer_id2): ProviderRecord(provider2, expired_timestamp)}
    ...
    async def test_find_providers_network_search(self):
        ...
        with patch.object(store, '_get_providers_from_peer', return_value=remote_providers):
            key = b"test_key"
            result = await store.find_providers(key)
    ...
    def test_concurrent_operations(self):
        ...
        for i in range(num_keys):
            ...
            for j in range(num_providers_per_key):
                ...
                peer_id = ID.from_base58(f"QmPeer{unique_id:06d}".replace('0', 'A') + "1" * 38)
                provider = PeerInfo(peer_id, [])
                store.add_provider(key, provider)
    ...
    def test_memory_efficiency_large_dataset(self):
        ...
        for key, providers in store.providers.items():
            for peer_id_str, record in providers.items():
                record.timestamp = current_time - PROVIDER_RECORD_EXPIRATION_INTERVAL - 1
    ...
    def test_expiration_boundary_conditions(self):
        ...
        timestamps = [
            ...
            current_time - PROVIDER_RECORD_REPUBLISH_INTERVAL + 1,  # No republish needed
