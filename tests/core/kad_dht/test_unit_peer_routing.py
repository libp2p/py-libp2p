...
class TestPeerRouting:
    ...
    def mock_host(self):
        ...
        host.get_connected_peers.return_value = []  # Default to empty list
    ...
    @pytest.mark.trio
    async def test_find_closest_peers_network_fallback_to_connected_peers(self, peer_routing, mock_host):
        """Test that network search falls back to connected peers when routing table has insufficient peers."""
        target_key = b"target_key"
    ...
    @pytest.mark.trio
    async def test_find_closest_peers_network_no_fallback_when_sufficient_peers(self, peer_routing, mock_host):
        """Test that network search does not fall back when routing table has sufficient peers."""
        target_key = b"target_key"
        ...
        with patch.object(
            ...
            with patch.object(peer_routing, "_query_peer_for_closest", return_value=[]):
                with patch(
                    ...
                    # Should not use connected peers as fallback, so sort_peer_ids_by_distance should not be called for fallback
                    # It will be called later in the method for sorting all candidates
                    assert result == sufficient_peers

    @pytest.mark.trio
    async def test_find_closest_peers_network_fallback_with_no_connected_peers(self, peer_routing, mock_host):
        """Test that network search handles case when no connected peers are available for fallback."""
        target_key = b"target_key"
        ...
        with patch.object(
            ...
            with patch.object(peer_routing, "_query_peer_for_closest", return_value=[]):
                with patch(
                    ...
                    # sort_peer_ids_by_distance will be called later in the method for sorting all candidates
