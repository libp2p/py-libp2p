...
class TestPeerRouting:
    ...
    async def test_find_closest_peers_network_empty_start(self, peer_routing):
        ...
        with patch.object(peer_routing.routing_table, 'find_local_closest_peers', return_value=[]):
    ...
    async def test_find_closest_peers_network_with_peers(self, peer_routing, mock_host):
        ...
        with patch.object(peer_routing.routing_table, 'find_local_closest_peers', return_value=initial_peers):
    ...
    async def test_find_closest_peers_convergence(self, peer_routing):
        ...
        with patch.object(peer_routing.routing_table, 'find_local_closest_peers', return_value=initial_peers):
            with patch.object(peer_routing, '_query_peer_for_closest', return_value=[]):
                with patch('libp2p.kad_dht.peer_routing.sort_peer_ids_by_distance', return_value=initial_peers):
                    result = await peer_routing.find_closest_peers_network(target_key)
                    
                    assert result == initial_peers

    @pytest.mark.trio
    async def test_query_peer_for_closest_success(self, peer_routing, mock_host, sample_peer_info):
        ...
        result = await peer_routing._query_peer_for_closest(sample_peer_info.peer_id, target_key)
    ...
    @pytest.mark.trio
    async def test_query_peer_for_closest_read_failure(self, peer_routing, mock_host, sample_peer_info):
    ...
    async def test_bootstrap_success(self, peer_routing, mock_host):
        ...
        with patch('libp2p.kad_dht.peer_routing.info_from_p2p_addr', side_effect=mock_peer_infos):
    ...
    async def test_bootstrap_connection_failure(self, peer_routing, mock_host):
        ...
        mock_peer_info = PeerInfo(
            create_valid_peer_id("bootstrap"),
            [Multiaddr("/ip4/127.0.0.1/tcp/8001")]
        )
        
        with patch('libp2p.kad_dht.peer_routing.info_from_p2p_addr', return_value=mock_peer_info):
            mock_host.get_peerstore().add_addrs = Mock()
    ...
    async def test_refresh_routing_table(self, peer_routing, mock_host):
        ...
        with patch.object(peer_routing, 'find_closest_peers_network', return_value=discovered_peers):
            ...
            peer_routing.find_closest_peers_network.assert_called_once_with(local_id.to_bytes())
    ...
    async def test_handle_kad_stream_find_node(self, peer_routing, mock_host):
        ...
        with patch.object(peer_routing.routing_table, 'find_local_closest_peers', return_value=closest_peers):
            mock_host.get_peerstore().addrs.return_value = [Multiaddr("/ip4/127.0.0.1/tcp/8004")]
    ...
    async def test_query_single_peer_for_closest_success(self, peer_routing):
        ...
        with patch.object(peer_routing, '_query_peer_for_closest', return_value=mock_result):
            await peer_routing._query_single_peer_for_closest(peer_id, target_key, new_peers)
    ...
    async def test_query_single_peer_for_closest_failure(self, peer_routing):
        ...
        # Mock query failure
        with patch.object(peer_routing, '_query_peer_for_closest', side_effect=Exception("Query failed")):
            await peer_routing._query_single_peer_for_closest(peer_id, target_key, new_peers)
    ...
    async def test_edge_case_max_rounds_reached(self, peer_routing):
        ...
        with patch.object(peer_routing.routing_table, 'find_local_closest_peers', return_value=initial_peers):
            with patch.object(peer_routing, '_query_peer_for_closest', side_effect=mock_query_side_effect):
                with patch('libp2p.kad_dht.peer_routing.sort_peer_ids_by_distance') as mock_sort:
    ...
    async def test_error_handling_in_bootstrap_parsing(self, peer_routing):
        ...
        with patch('libp2p.kad_dht.peer_routing.info_from_p2p_addr', side_effect=Exception("Parse error")):
