...
@pytest.mark.trio
async def test_select_relay_no_relays(circuit_v2_transport):
...
@pytest.mark.trio
async def test_select_relay_all_unavailable(circuit_v2_transport, peer_info):
...
@pytest.mark.trio
async def test_select_relay_round_robin(circuit_v2_transport, peer_info):
    """Test _select_relay round-robin selection of available relays."""
    with patch("trio.sleep", new=AsyncMock()) as mock_sleep:
        relay1 = MagicMock(spec=ID)
        ...
        circuit_v2_transport._is_relay_available = AsyncMock(return_value=True)  # Use return_value for consistent results
        
        try:
            # First call should return relay1
            result1 = await circuit_v2_transport._select_relay(peer_info)
            assert result1 == relay1, f"Expected relay1, got {result1}"
            assert circuit_v2_transport._last_relay_index == 0, f"Expected index 0, got {circuit_v2_transport._last_relay_index}"
            
            # Second call should return relay2 (round-robin)
            result2 = await circuit_v2_transport._select_relay(peer_info)
            assert result2 == relay2, f"Expected relay2, got {result2}"
            assert circuit_v2_transport._last_relay_index == 1, f"Expected index 1, got {circuit_v2_transport._last_relay_index}"
            
            # Third call should return relay1 again
            result3 = await circuit_v2_transport._select_relay(peer_info)
            assert result3 == relay1, f"Expected relay1, got {result3}"
            assert circuit_v2_transport._last_relay_index == 0, f"Expected index 0, got {circuit_v2_transport._last_relay_index}"
            
            # Ensure trio.sleep was not called (since relays are available)
            assert mock_sleep.call_count == 0, f"Expected no calls to trio.sleep, got {mock_sleep.call_count}"
        
        except Exception as e:
            print(f"Unexpected error in test_select_relay_round_robin: {str(e)}")
            raise

@pytest.mark.trio
async def test_is_relay_available_success(circuit_v2_transport):
...
@pytest.mark.trio
async def test_is_relay_available_failure(circuit_v2_transport):
    """Test _is_relay_available when the relay is unreachable."""
    relay_id = MagicMock(spec=ID)
    circuit_v2_transport.host.new_stream = AsyncMock(side_effect=Exception("Connection failed"))
