...
def test_seed_must_be_32_bytes():
    ...
    # Too long
    with pytest.raises(ValueError, match="must be exactly 32 bytes"):
        create_identity_from_seed(b"this_seed_is_way_too_long_for_ed25519_key_generation")
...
def test_save_and_load_identity():
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert loaded_key_pair.private_key.to_bytes() == original_key_pair.private_key.to_bytes()
        assert loaded_key_pair.public_key.to_bytes() == original_key_pair.public_key.to_bytes()
