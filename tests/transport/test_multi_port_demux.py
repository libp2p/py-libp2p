...
async def test_multi_port_demux():
    ...
    """
    server = new_host(enable_quic=False, enable_websocket=True, listen_addrs=[
    ...
    async with server.run(listen_addrs=[
        ...
        assert tcp_port == ws_port, f"Expected same port for TCP and WS, got {tcp_port} and {ws_port}"

    # Now listen on two explicitly different ports to ensure they don't overwrite each other
    # or silently break.
    server2 = new_host(enable_quic=False, enable_websocket=True, listen_addrs=[
    ...
    async with server2.run(listen_addrs=[
        Multiaddr("/ip4/127.0.0.1/tcp/43511"),
        Multiaddr("/ip4/127.0.0.1/tcp/43511/ws"),
        Multiaddr("/ip4/127.0.0.1/tcp/43512"),
        Multiaddr("/ip4/127.0.0.1/tcp/43512/ws"),
        ...
        async with client1.run(listen_addrs=[]):
            await client1.connect(PeerInfo(server2.get_id(), [Multiaddr("/ip4/127.0.0.1/tcp/43511")]))
        ...
        async with client2.run(listen_addrs=[]):
            await client2.connect(PeerInfo(server2.get_id(), [Multiaddr("/ip4/127.0.0.1/tcp/43512")]))
