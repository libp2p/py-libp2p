...
1. **No Peer Scoring** — cannot optimize which peers to request from
1. **No Rebroadcast Logic** — if initial request fails, content is never retrieved unless manually retried
1. **Race Conditions in `_pending_requests`** — concurrent operations on shared state without proper isolation
1. **Unbounded Memory Growth** — `_dont_have_responses`, `_expected_blocks`, `_have_confirmed` lack cleanup
